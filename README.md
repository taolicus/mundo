...
- ✅ **Travel-reason rebalance done (leisure rare).** `ExplorationNeed` gates its intent at a high `explorationThreshold` (0.9, only fires near full maturity) and is a **stable per-dweller trait** (`isCurious` rolled once at construction, `chance(explorationProb:0.35)`) — not re-rolled per travel. `onArrival` no longer wipes needs (needs are place-agnostic now that hunger is substitutable), so wanderlust and hunger accumulate across travel instead of resetting. Gather stays frequent (180-480) so resource travel is visibly busy; sampled over 60k ticks: **curiosity ~1.2%** of trips, resources ~98.8% (gather + seeking), travelers in-transit on ~98% of ticks — travel is mostly for resources with leisure as a rare occasion, per design intent.
